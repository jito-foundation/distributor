...
RUN --mount=type=cache,mode=0777,target=/home/root/app/target \
    ...
    cargo build --release --bin jito-airdrop-api && cp target/release/jito-airdrop-api ./
...
FROM base_image as jito-airdrop-api
WORKDIR /app
COPY --from=builder /home/root/app/jito-airdrop-api ./
ENTRYPOINT ["./jito-airdrop-api"]
